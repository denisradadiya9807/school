...
exports.list = async (req, res) => {
    ...
    if (req.token && mongoose.Types.ObjectId.isValid(req.token.adminId)) {
        ...
        if (academicdata && academicdata != null && academicdata.status === true) {
            let havepermissions = await config.getsuperadminPermission(req, academicdata.roleid, 'day', 'View');
            if (havepermissions) {
                let branch = await primary.model(constants.supermodel.day, daymodel).find({ dayname: { '$regex': new RegExp(search, "i") }, });
...
exports.withpagination = async (req, res) => {
    ...
    if (req.token && mongoose.Types.ObjectId.isValid(req.token.adminId)) {
        ...
        if (branchdata && branchdata != null && branchdata.status === true) {
            let havepermissions = await config.getsuperadminPermission(req, branchdata.roleid, 'day', 'View');
            if (havepermissions) {
                primary.model(constants.supermodel.day, daymodel).paginate({}, {
                    page,
