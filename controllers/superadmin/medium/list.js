...
exports.list = async (req, res) => {
    ...
    if (req.token && mongoose.Types.ObjectId.isValid(req.token.adminId)) {
        ...
        if (academicdata && academicdata != null && academicdata.status === true) {
            let havepermissions = await config.getsuperadminPermission(req, academicdata.roleid, 'medium', 'View');
            if (havepermissions) {
                let branch = await primary.model(constants.supermodel.medium, mediummodel).find({ mediumname: { '$regex': new RegExp(search, "i") }, });
...
exports.withpagination = async (req, res) => {
    ...
    if (req.token && mongoose.Types.ObjectId.isValid(req.token.adminId)) {
        ...
        if (branchdata && branchdata != null && branchdata.status === true) {
            let havepermissions = await config.getsuperadminPermission(req, branchdata.roleid, 'medium', 'View');
            if (havepermissions) {
                primary.model(constants.supermodel.medium, mediummodel).paginate({}, {
                    page,
