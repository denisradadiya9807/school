...
exports.save = async (req, res) => {
    const { mediumname, status, mediumid, branchid } = req.body
    if (req.token && mongoose.Types.ObjectId.isValid(req.token.adminId)) {
        ...
        if (branch && branch != null && branch.status === true) {
            let havepermissions = await config.getsuperadminPermission(req, branch.roleid, 'medium', 'insertupdate');
            if (havepermissions) {
                const existing = await primary.model(constants.supermodel.medium, mediummodel).findOne({ mediumname: mediumname }).lean();
                if (existing) {
                ...
                if (mediumid) {
                    if (!mongoose.Types.ObjectId.isValid(mediumid)) {
                        return responsemanager.onBadRequest({ message: 'mediumid is not valid' }, res);
                    }
                    let branchupdate = await primary.model(constants.supermodel.medium, mediummodel)
                        .findOneAndUpdate(
                            { _id: new mongoose.Types.ObjectId(mediumid) },
                            obj,
                    ...
                    let academic = await primary.model(constants.supermodel.medium, mediummodel).create(obj);
                    return responsemanager.onSuccess('medium Create Successfully...', academic, res);
                }
            } else {
