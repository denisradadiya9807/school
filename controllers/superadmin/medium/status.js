...
exports.status = async (req, res) => {
    ...
    if (req.token && mongoose.Types.ObjectId.isValid(req.token.adminId)) {
        ...
        if (branchdata && branchdata != null && branchdata.status === true) {
            ...
            if (havepermissions) {
                let branch = await primary.model(constants.supermodel.medium, mediummodel).findById({ _id: new mongoose.Types.ObjectId(mediumid) });
                let obj = {
                    status: (branch && branch.status === true) ? false : true
                }
                let changestatus = await primary.model(constants.supermodel.medium, mediummodel).findByIdAndUpdate(mediumid, obj);
