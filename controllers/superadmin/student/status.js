...
exports.status = async (req, res) => {
    ...
    if (req.token && mongoose.Types.ObjectId.isValid(req.token.adminId)) {
        ...
        if (branchdata && branchdata != null && branchdata.status === true) {
            let havepermissions = await config.getsuperadminPermission(req, branchdata.roleid, 'student', 'View');
            if (havepermissions) {
                let branch = await primary.model(constants.supermodel.student, studentsmodel).findById({ _id: new mongoose.Types.ObjectId(studentid) });
                let obj = {
                    status: (branch && branch.status === true) ? false : true
                }
                let changestatus = await primary.model(constants.supermodel.student, studentsmodel).findByIdAndUpdate(studentid, obj);
