...
exports.status = async (req, res) => {
    ...
    if (req.token && mongoose.Types.ObjectId.isValid(req.token.adminId)) {
        ...
        if (branchdata && branchdata != null && branchdata.status === true) {
            ...
            if (havepermissions) {
                let branch = await primary.model(constants.supermodel.class, classmodel).findById({ _id: new mongoose.Types.ObjectId(classid) });
                let obj = {
                    status: (branch && branch.status === true) ? false : true
                }
                let changestatus = await primary.model(constants.supermodel.class, classmodel).findByIdAndUpdate(classid, obj);
