...
exports.status = async (req, res) => {
    ...
    if (req.token && mongoose.Types.ObjectId.isValid(req.token.adminId)) {
        ...
        if (branchdata && branchdata != null && branchdata.status === true) {
            let havepermissions = await config.getsuperadminPermission(req, branchdata.roleid, 'teacher', 'View');
            if (havepermissions) {
                let branch = await primary.model(constants.supermodel.teacher, teachermodel).findById({ _id: new mongoose.Types.ObjectId(teacherid) });
                let obj = {
                    status: (branch && branch.status === true) ? false : true
                }
                let changestatus = await primary.model(constants.supermodel.teacher, teachermodel).findByIdAndUpdate(teacherid, obj);
