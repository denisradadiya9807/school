...
exports.list = async (req, res) => {
    ...
    if (req.token && mongoose.Types.ObjectId.isValid(req.token.adminId)) {
        ...
        if (academicdata && academicdata != null && academicdata.status === true) {
            let havepermissions = await config.getsuperadminPermission(req, academicdata.roleid, 'semester', 'View');
            if (havepermissions) {
                let branch = await primary.model(constants.supermodel.semester, semestermodel).find({ nameofsemester: { '$regex': new RegExp(search, "i") }, });
...
exports.withpagination = async (req, res) => {
    ...
    if (req.token && mongoose.Types.ObjectId.isValid(req.token.adminId)) {
        ...
        if (branchdata && branchdata != null && branchdata.status === true) {
            ...
            if (havepermissions) {
                primary.model(constants.supermodel.semester, semestermodel).paginate({}, {
                    page,
                    limit: parseInt(limit),
                    sort: { _id: -1 },
                    lean: true
                }).then((branchdata) => {
                    return responsemanager.onSuccess('semester Data...', branchdata, res);
                }).catch((error) => {
