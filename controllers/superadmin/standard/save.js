...
exports.save = async (req, res) => {
    const { standardname, status, standardid, branchid } = req.body
    if (req.token && mongoose.Types.ObjectId.isValid(req.token.adminId)) {
        ...
        if (branch && branch != null && branch.status === true) {
            let havepermissions = await config.getsuperadminPermission(req, branch.roleid, 'standard', 'insertupdate');
            if (havepermissions) {
                const existing = await primary.model(constants.supermodel.standard, standardmodel).findOne({ standardname: standardname }).lean();
                if (existing) {
                ...
                if (standardid) {
                    if (!mongoose.Types.ObjectId.isValid(standardid)) {
                        return responsemanager.onBadRequest({ message: 'standardid is not valid' }, res);
                    }
                    let branchupdate = await primary.model(constants.supermodel.standard, standardmodel)
                        .findOneAndUpdate(
                            { _id: new mongoose.Types.ObjectId(standardid) },
                            obj,
                    ...
                    let academic = await primary.model(constants.supermodel.standard, standardmodel).create(obj);
                    return responsemanager.onSuccess('standard Create Successfully...', academic, res);
                }
            } else {
