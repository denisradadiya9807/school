...
exports.save = async (req, res) => {
    const { boardname, status, boardid, branchid } = req.body
    if (req.token && mongoose.Types.ObjectId.isValid(req.token.adminId)) {
        ...
        if (branch && branch != null && branch.status === true) {
            let havepermissions = await config.getsuperadminPermission(req, branch.roleid, 'board', 'insertupdate');
            if (havepermissions) {
                const existing = await primary.model(constants.supermodel.board, boardmodel).findOne({ boardname: boardname }).lean();
                if (existing) {
                ...
                if (boardid) {
                    if (!mongoose.Types.ObjectId.isValid(boardid)) {
                        return responsemanager.onBadRequest({ message: 'boardid is not valid' }, res);
                    }
                    let branchupdate = await primary.model(constants.supermodel.board, boardmodel)
                        .findOneAndUpdate(
                    ...
                    let academic = await primary.model(constants.supermodel.board, boardmodel).create(obj);
                    return responsemanager.onSuccess('board Create Successfully...', academic, res);
                }
            } else {
