...
const boardnmodel = require('../../../models/board.model');
...
exports.list = async (req, res) => {
    ...
    if (req.token && mongoose.Types.ObjectId.isValid(req.token.adminId)) {
        ...
        if (academicdata && academicdata != null && academicdata.status === true) {
            let havepermissions = await config.getsuperadminPermission(req, academicdata.roleid, 'board', 'View');
            if (havepermissions) {
                let branch = await primary.model(constants.supermodel.board, boardnmodel).find({ boardname: { '$regex': new RegExp(search, "i") }, });
...
exports.withpagination = async (req, res) => {
    ...
    if (req.token && mongoose.Types.ObjectId.isValid(req.token.adminId)) {
        ...
        if (branchdata && branchdata != null && branchdata.status === true) {
            let havepermissions = await config.getsuperadminPermission(req, branchdata.roleid, 'boardn', 'View');
            if (havepermissions) {
                primary.model(constants.supermodel.board, boardnmodel).paginate({}, {
                    page,
