...
exports.list = async (req, res) => {
    ...
    if (req.token && mongoose.Types.ObjectId.isValid(req.token.adminId)) {
        ...
        if (adminData && adminData != null && adminData.status === true) {
            ...
            if (getpermission) {
                let schooldata = await primary.model(constant.Model.school, schoolmodel).find({ schoolname: { '$regex': new RegExp(search, "i") }, }).lean();
                return responseManager.onSuccess('school list...', schooldata, res);
            } else {
